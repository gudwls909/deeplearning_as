...
def conv_forward(x, w, b, conv_param):
    ...
    if padding == 'valid':
        ...
        H_out = 1 + (H - WH + WH - 1) // SH  # = 1 + (H - 1) / SH = ceil(H / SH)
        W_out = 1 + (W - WW + WW - 1) // SW  # = 1 + (W - 1) / SW = ceil(W / SW)
        u_pad = (WH - 1) // 2
        d_pad = WH // 2
        l_pad = (WW - 1) // 2
        r_pad = WW // 2
        x = np.pad(x, ((0, 0), (u_pad, d_pad), (l_pad, r_pad), (0, 0)), 'constant', constant_values=0)
    
    out = np.zeros([N, H_out, W_out, F])
    for datapt_idx in range(N):
        for filter_idx in range(F):
            for hpos in range(H_out):
                for wpos in range(W_out):
                    starth = hpos * SH
                    startw = wpos * SW
                    # dot produce the filter over all channels
                    subarray = x[datapt_idx, starth:starth+WH, startw:startw+WW, :]
                    subfilter = w[filter_idx, :, :, :]
                    dotproduct = subarray.flatten().dot(subfilter.flatten())
                    out[(datapt_idx, hpos, wpos, filter_idx)] = dotproduct + b[filter_idx]

    if padding == 'same':
        x = x[:, u_pad:-d_pad, l_pad: -r_pad, :]
    pass
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def conv_backward(dout, cache):
    ...
    if padding == 'valid':
        ...
        r_pad = WW // 2
        x = np.pad(x, ((0, 0), (u_pad, d_pad), (l_pad, r_pad), (0, 0)), 'constant', constant_values=0)
        #dout = np.pad(dout, ((0, 0), (u_pad, d_pad), (l_pad, r_pad), (0, 0)), 'constant', constant_values=0)
        ...
        for filter_idx in range(F):
            for hpos in range(H_out):
                for wpos in range(W_out):
                    ...
                    # dot produce the filter over all channels
                    subarray = x[datapt_idx, starth:starth+WH, startw:startw+WW, :]
                    dw[filter_idx, :, :, :] += subarray * dout[datapt_idx, hpos, wpos, filter_idx]
                    dx[datapt_idx, starth:starth+WH, startw:startw+WW, :] += w[filter_idx, :, :, :] * dout[datapt_idx, hpos, wpos, filter_idx]

    if padding == 'same':
        dx = dx[:, u_pad:-d_pad, l_pad: -r_pad, :]
...
def max_pool_forward(x, pool_param):
    ...
    H_out = 1 + (H - pool_height) // SH
    W_out = 1 + (W - pool_width) // SW
        ...
        for c_idx in range(C):
            for hpos in range(H_out):
                for wpos in range(W_out):
                    starth = hpos * SH
                    startw = wpos * SW
                    subarray = x[datapt_idx, starth:starth+pool_height, startw:startw+pool_width, c_idx]
                    out[datapt_idx, hpos, wpos, c_idx] = subarray.max()
        ...
        for c_idx in range(C):
            for hpos in range(H_out):
                for wpos in range(W_out):
                    ...
                    startw = wpos * SW
                    subarray = x[datapt_idx, starth:starth+pool_height, startw:startw+pool_width, c_idx]
                    max_idx = (subarray == np.max(subarray))
                    dx[datapt_idx, starth:starth+pool_height, startw:startw+pool_width, c_idx] += max_idx * dout[datapt_idx, hpos, wpos, c_idx]
